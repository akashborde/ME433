...
void APP_Tasks(void) {
            ...
            IMU_read_multiple(OUT_TEMP_L, appData.dat, NUM_READS);

            appData.x_accel =   (appData.dat[9] << 8) | appData.dat[8];
            appData.y_accel =   (appData.dat[11] << 8) | appData.dat[10];

            appData.horiz_vel = accel_to_percent(appData.x_accel);
            appData.vert_vel = accel_to_percent(appData.y_accel);
            
            //deadband
            if(appData.horiz_vel < DEADBAND) {appData.horiz_vel = 0;}
            if(appData.vert_vel < DEADBAND) {appData.vert_vel = 0;}
            
            //direction
            if(appData.x_accel < 0) {appData.horiz_vel *= -1;}
            if(appData.y_accel < 0) {appData.vert_vel *= -1;}
            ...
            sprintf(appData.m, "y_accel = %d  ", appData.y_accel);
            LCD_print(appData.m, START_X, START_Y + 20, ILI9341_BLUE, ILI9341_PINK);
            sprintf(appData.m, "x_accel_p = %d  ", accel_to_percent(appData.x_accel));
            LCD_print(appData.m, START_X, START_Y + 30, ILI9341_BLUE, ILI9341_PINK);
            sprintf(appData.m, "y_accel_p = %d  ", accel_to_percent(appData.y_accel));
            LCD_print(appData.m, START_X, START_Y + 40, ILI9341_BLUE, ILI9341_PINK);

            
            LCD_horizProgBar(CENTER_X, CENTER_Y + 50, 50, 5, appData.x_accel,
                    ILI9341_ORANGE, ILI9341_BLACK);
            LCD_vertProgBar(CENTER_X - 50, CENTER_Y, 5, 50, appData.y_accel,
                    ILI9341_ORANGE, ILI9341_BLACK);
...
//            {
                    ...
                    appData.xCoordinate = (int8_t)  2*appData.horiz_vel;
                    appData.yCoordinate = (int8_t)  2*appData.vert_vel;
                    movement_length = 0;
//            }
//            else
//            {
